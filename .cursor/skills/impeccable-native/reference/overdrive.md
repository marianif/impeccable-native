...
2. **ask the user directly to clarify what you cannot infer.** to present these directions and get explicit user approval before writing any code. For each direction, name the trade-offs: performance cost on mid-tier Android, dependency weight, fallback quality when reduced motion is on, and which register it fits.
...
1. **Flavor detector**: `node .cursor/skills/impeccable-native/scripts/detect-rn-flavor.mjs`. You need:
...
If anything is unclear from the codebase, ask the user directly to clarify what you cannot infer.
...
After each pass, verify on **both** iOS Simulator and Android Emulator. For gesture effects, test on a real device — simulators lie about gesture velocity and frame timing. Capture screenshots and screen recordings via `node .cursor/skills/impeccable-native/scripts/screenshot.mjs`.
