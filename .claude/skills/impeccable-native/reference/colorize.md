...
3. **Brand inputs**: existing accent, anti-references (e.g. "not another Material 3 default", "not a Stripe-cream finance app"), and `platform-fidelity`. If unclear, STOP and call the AskUserQuestion tool to clarify..
...
Screenshot both simulators (`node .claude/skills/impeccable-native/scripts/screenshot.mjs`) at the same screen, light and dark, side by side. If the dark-mode capture looks like "light mode with the lights off," redesign the dark palette — depth comes from lightness, not darkness.
