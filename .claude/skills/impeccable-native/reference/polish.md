...
Before polishing: confirm the flavor detector has run (`node .claude/skills/impeccable-native/scripts/detect-rn-flavor.mjs`) and that `load-context.mjs` surfaced PRODUCT.md + DESIGN.md. `Platform Fidelity` and `Primary Devices` define the bar for what polish even means on this project.
...
4. **Pull in any prior critique** (optional signal): if `/impeccable-native critique` has been run on the same target, its priority issues are a useful prior for what to address first. Resolve the target to a file path, then:
   ```bash
   slug=$(node .claude/skills/impeccable-native/scripts/critique-storage.mjs slug "<resolved-path>")
   node .claude/skills/impeccable-native/scripts/critique-storage.mjs latest "$slug"
   ```
...
Work through these dimensions methodically. Verify each pass on iOS Simulator and Android Emulator. Capture before/after via `node .claude/skills/impeccable-native/scripts/screenshot.mjs` when changes are visual.
...
- **Capture screenshots** of the polished state on both platforms via `node .claude/skills/impeccable-native/scripts/screenshot.mjs` for the record.
...
- **Consolidate tokens**: if you introduced new color, space, or duration values, check whether they should be added to `tokens.ts`. Run `node .claude/skills/impeccable-native/scripts/extract-tokens.mjs --dry-run` on the polished file to surface duplicated literals.
