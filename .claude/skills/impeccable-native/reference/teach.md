...
If the signal is split (e.g. an app with both a marketing onboarding and a product core), STOP and call the AskUserQuestion tool to clarify. Ask which register describes the **primary** surface.
...
**Critical: re-run the loader to refresh session context.** After writing PRODUCT.md, run `node .claude/skills/impeccable-native/scripts/load-context.mjs` one final time and let its full JSON output land in conversation. This ensures subsequent commands in this session use the freshly-written PRODUCT.md, not a stale version.
