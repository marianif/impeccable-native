...
The **recommendation logic is shared library code** — `.claude/skills/impeccable-native/scripts/design-system/state-rules.mjs`. Synthesized scripts import it; the decision table stays uniform across all repos. Fixing a bug in the rules fixes it everywhere.
...
```
node .claude/skills/impeccable-native/scripts/design-system/detect-layout.mjs --dir=.
```
...
```
node .claude/skills/impeccable-native/scripts/design-system/inventory.mjs --dir=. [--force]
```
