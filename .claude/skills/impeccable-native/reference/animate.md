...
1. **Run the flavor detector** (`node .claude/skills/impeccable-native/scripts/detect-rn-flavor.mjs`). You need to know:
...
After each implementation pass, walk the surface on **both** iOS Simulator and Android Emulator. Capture before/after via `node .claude/skills/impeccable-native/scripts/screenshot.mjs` when changes are visual. Recording is better than screenshots for motion — use the simulator screen recording.
...
When the motion clarifies state, settles with physics, respects reduced-motion, and feels equally at home on iPhone and Pixel, hand off to `/impeccable-native polish` for the final pass.
