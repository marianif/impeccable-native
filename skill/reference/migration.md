...
```bash
node {{scripts_path}}/migration/migration-scan.mjs --scope=<scope> --dir=.
```
...
2. Gate on `{{command_prefix}}impeccable-native audit <file>` for each migrated file — the migration is not allowed to regress touch targets, contrast, or platform parity even if the new design was approved.
