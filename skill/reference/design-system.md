...
The **recommendation logic is shared library code** — `{{scripts_path}}/design-system/state-rules.mjs`. Synthesized scripts import it; the decision table stays uniform across all repos. Fixing a bug in the rules fixes it everywhere.
...
```
node {{scripts_path}}/design-system/detect-layout.mjs --dir=.
```
...
```
node {{scripts_path}}/design-system/inventory.mjs --dir=. [--force]
```
