...
`break` is the one command with permission to question the system itself — it metaphorically *breaks* the design system to escape its gravity. Every other refine command (`polish`, `quieter`, `bolder`, `distill`, `rethink`) works *within* the current design system and respects its tokens. Use `break` when the developer says the component's design is uninspired, derivative, or wrong at the concept level — *and* the existing tokens are part of what's wrong.

This is a heavy command. If the user wants the component cleaner, calmer, or louder, redirect to `distill`, `quieter`, or `bolder`. If the brand is fine but the component doesn't earn its place in the flow, redirect to `rethink` (purpose-driven redesign within the system). Confirm the scope is genuinely "burn it down" before spending the effort.
...
```bash
node {{scripts_path}}/break-scan.mjs --target=<path-to-component> --dir=.
```
...
First, design the right answer without looking at the current tokens. This is the radical step — the point of `break` is to escape the existing system's gravity.
...
- Run the **category-reflex check** at both altitudes. A break that lands on the first training-data reflex for the category has failed its only job.
...
After applying, offer `{{command_prefix}}impeccable-native audit <target>` to verify the redesign holds up on touch targets, contrast, and platform parity.
