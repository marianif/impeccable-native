...
3. **Brand inputs**: existing accent, anti-references (e.g. "not another Material 3 default", "not a Stripe-cream finance app"), and `platform-fidelity`. If unclear, {{ask_instruction}}.
...
Screenshot both simulators (`node {{scripts_path}}/screenshot.mjs`) at the same screen, light and dark, side by side. If the dark-mode capture looks like "light mode with the lights off," redesign the dark palette — depth comes from lightness, not darkness.

When the palette earns its place and tokens are clean, hand off to `{{command_prefix}}impeccable-native polish` for the final pass.
