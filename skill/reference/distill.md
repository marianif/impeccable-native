...
If any of these are unclear from the codebase or PRODUCT.md, {{ask_instruction}}
...
- **Consolidate `StyleSheet.create` blocks**: merge similar styles within a file, lift truly shared styles into a shared module, dedupe literals into `tokens.ts`. Run `node {{scripts_path}}/extract-tokens.mjs --dry-run` to surface duplicate literals across files; promote the ones that appear 3+ times.
...
- **Tokens compact**: re-run `node {{scripts_path}}/extract-tokens.mjs --dry-run` after the pass — duplicate literals should have dropped.
...
When the cuts feel right, hand off to `{{command_prefix}}impeccable-native polish` for the final pass. As Antoine de Saint-Exupéry put it: "Perfection is achieved not when there is nothing more to add, but when there is nothing left to take away."
