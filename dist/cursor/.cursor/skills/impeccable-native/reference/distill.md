...
- **Consolidate `StyleSheet.create` blocks**: merge similar styles within a file, lift truly shared styles into a shared module, dedupe literals into `tokens.ts`. Run `node .cursor/skills/impeccable-native/scripts/extract-tokens.mjs --dry-run` to surface duplicate literals across files; promote the ones that appear 3+ times.
...
- **Tokens compact**: re-run `node .cursor/skills/impeccable-native/scripts/extract-tokens.mjs --dry-run` after the pass — duplicate literals should have dropped.
