...
2. **Compute the slug**:
   ```bash
   node .cursor/skills/impeccable-native/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
CLI scan:
```bash
node .cursor/skills/impeccable-native/scripts/detect.mjs --json [--fast] [target]
```
...
4. If mutation is available, start `node .cursor/skills/impeccable-native/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
