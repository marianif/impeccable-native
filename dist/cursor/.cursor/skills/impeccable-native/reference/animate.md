...
1. **Run the flavor detector** (`node .cursor/skills/impeccable-native/scripts/detect-rn-flavor.mjs`). You need to know:
...
After each implementation pass, walk the surface on **both** iOS Simulator and Android Emulator. Capture before/after via `node .cursor/skills/impeccable-native/scripts/screenshot.mjs` when changes are visual. Recording is better than screenshots for motion — use the simulator screen recording.
