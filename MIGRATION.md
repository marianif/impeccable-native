...
- ✅ Done: 18 (`teach`, `shape`, `craft`, `document`, `audit`, `critique`, `polish`, `bolder`, `quieter`, `distill`, `harden`, `onboard`, `codex`, `spatial-design`, `typography`, `motion-design`, `color-and-contrast`, `interaction-design`)
- 🟡 In progress: 0
- 🔵 No-op (verify only): 5
- ⬜ Todo: 11
- ❌ Removed: 1 (`live`)

**Next up:** enhance set — `animate` → `colorize` → `typeset` → `layout` → `delight` → `overdrive`. Then fix set (`clarify` → `adapt` → `optimize`), build leftover (`extract`), register references (`brand`, `product`), then resolve `responsive-design.md` fate (fold into `adapt.md` or keep).
