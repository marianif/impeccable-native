...
| File | Status | Notes |
|------|:------:|-------|
| `live.md` | ❌ | Removed. RN has no browser runtime. Replaced by `screenshot.mjs` for simulator iteration (in progress — see Scripts). |

---

## Scripts (IMPECCABLE.md Phase 5)

RN-specific tooling under `skill/scripts/`. The web `live-*.mjs` subsystem is dropped entirely; these replace it with static analysis + simulator capture.

| Script | Status | Notes |
|--------|:------:|-------|
| `detect-rn-flavor.mjs` | ✅ | 183 lines. Reads `package.json` (+ `app.json`/`app.config.*`). Returns `{ flavor, router, styling, newArch, sdkVersion, rnVersion }`. JSON to stdout, helpful stderr on failure. Consumed by command references at Step 0. |
